...
export default function Navbar() {
    ...
    return (
        <>
            ...
            >
                <Link to='/'>
                <img src="/favicon.svg" alt="logo" className="h-9 w-auto" /> 
                </Link>

                <div className="hidden md:flex items-center gap-8 transition duration-500">
                    <Link to='/' onClick={() => window.scrollTo({ top: 0, behavior: "smooth" })} className="hover:text-pink-300 transition">Home</Link>
                    <Link to='/generate' className="hover:text-pink-300 transition">Generate</Link>
                    {
                        isLoggedIn ? <Link to='/my-generation' className="hover:text-pink-300 transition">My Generation</Link>
            ...
            <div className={`fixed inset-0 z-100 bg-black/40 backdrop-blur flex flex-col items-center justify-center text-lg gap-8 md:hidden transition-transform duration-400 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
                <Link onClick={() => { setIsOpen(false); window.scrollTo({ top: 0, behavior: 'smooth' }); }} to='/'>Home</Link>
